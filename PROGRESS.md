...
- Decision: keep option (2) — preparation continues from the published branch — and do **not** force the delivery push, in any form.
  ...
  - Evidence that (1) is not a safe interim, and that the two repository modes disagree about it in opposite directions (plain git 2.43.0, both reproduced):
    - `existing_path`: `--force-with-lease` given no expected value reads the checkout's own `refs/remotes/origin/<branch>`, which the runner's **own previous push** already advanced to the exact tip it is about to overwrite. The lease is therefore satisfied by construction and the force goes through: ` + 4cee993...a83c25e agent/x -> agent/x (forced update)`, after which `git cat-file -e refs/heads/agent/x:a.txt` fails — the first delivery is gone. The lease guards against a push the runner never saw; it does not guard against the runner's own stale base, which is exactly what this defect was.
    - `managed_clone`: there is no remote-tracking ref for the lease to read at all — a mirror cache keeps the remote's branches as its own `refs/heads/*` — so the same push is refused outright: `! [rejected] agent/y -> agent/y (stale info)`. Option (1) would not have fixed the symptom in the **default** repository mode; it would only have renamed the rejection.
...
- Issue: two concurrent executions of the *same* job publishing to the same branch is still resolved by one of them failing.
  ...
  - Suggested resolution: none at the runner. If the orchestrator ever leases two executions of one job concurrently, that is the thing to fix.

## Next Recommended Implementation

- Land the `runner/README.md` correction above once PR #164 is merged.
