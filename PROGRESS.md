...
- [x] A half-open circuit can no longer be left without a live probe
  ...
  - Behavior delivered:
    ...
    - Every writer shares `orchestrator/src/moirai/persistence/circuits.py`, so the transition, offer-release and reaper paths cannot drift apart again.
  ...
  - Validation performed: ten PostgreSQL integration tests written first and confirmed failing against `origin/main`, then confirmed passing; eighteen unit tests; full orchestrator suite; lint; type check.
  - Commands executed (throwaway PostgreSQL 16 container on port 55492, removed afterwards):
    ...
    - `make test-orchestrator` → `Ran 386 tests … OK (skipped=19)`.
    - `make lint` → `All checks passed!`
...
- **Regression, now fixed.** Moving the provider-circuit verdict out of the per-project loop removed the *clear* that used to happen on every healthy project's sync, but kept recording a failure per failing project. One permanently broken project beside a healthy one — a deleted repository, a bad URL — therefore accumulated a provider failure every pass with nothing clearing it, opening the shared circuit on the third pass and halting scheduling for every project on GitHub. `main` did not do this. The verdict is now provider-wide (see Decisions): any project that syncs clears the circuit, only a pass in which every attempted project failed records a failure, and it records one. Reproductions: `test_a_permanently_broken_project_never_accumulates_provider_failures` and `test_one_broken_project_never_opens_the_shared_provider_circuit`, both confirmed failing against the intermediate design (`AssertionError: Lists differ: [('github', 'issue tracker failed for proj…')] != []`).
...
- Decision: the shared provider circuit is decided once per sync pass, from provider-wide evidence only.
  ...
  - Alternatives considered: (a) keep per-project clearing and rely on the next failing pass to reopen the circuit; (b) an intermediate form this session actually implemented first — clear only when the pass had a success *and* no failure, otherwise record per failing project.
  - Reason: the row is global, so only a whole-pass verdict about *the provider* is meaningful. (b) was wrong in the other direction and my own adversarial review caught it before the PR: one permanently broken project (deleted repository, bad URL) beside healthy ones records a provider failure every pass with nothing ever clearing it, so the shared circuit opens after three passes and halts scheduling for every project on GitHub — a regression against the behaviour on `main`, where the healthy project's success cleared it each pass. A single project's fault is a project fault; it is already handled by that project's own `app.issue_sync_state` backoff.
  - Consequences: any project that syncs clears the circuit (the provider answered); only a pass in which every attempted project failed records a failure, and records exactly one; a pass where all projects are backing off writes nothing, because it made no request and learned nothing. A real provider outage still opens the circuit on the third consecutive pass, as before. A single-project deployment is unchanged: with one project, "every attempted project failed" and "this project failed" are the same statement.
...
- Targeted tests: Passed — 10 new PostgreSQL integration tests (`CircuitBreakerIntegrationTests`) and 18 new unit tests. All 10 integration tests were re-confirmed failing against `origin/main` at `30d8483` after the merge: `Ran 10 tests … FAILED (failures=8, errors=2)`.
- Service tests: Passed, after merging `origin/main` (`30d8483`, issue #90) — `make test-orchestrator` → `Ran 386 tests … OK (skipped=19)`; `LOOP_TEST_DATABASE_URL=… make test-postgres-integration` → `Ran 19 tests … OK` against a fresh database.
