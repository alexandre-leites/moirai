...
-- 'waiting_ai_review', 029 added 'repairing'); internal/server/status.go's
-- StatusPlanning, added by #351.
--
...
ALTER TABLE app.workflow_runs
  ADD CONSTRAINT workflow_runs_status_is_known CHECK (
    status IN ('offered', 'preparing', 'planning', 'waiting_github_checks', 'waiting_human', 'waiting_ai_review', 'repairing', 'delivering', 'completed', 'failed', 'blocked', 'cancelled')
  );
