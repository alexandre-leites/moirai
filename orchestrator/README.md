...
The advancing branch does not replay the state updates that rode on the outbox row, so a gate the lost invocation would have set (`plan_valid`, `pipeline_passed`, `review_approved`) stays as it was and the graph re-runs that phase rather than advancing on a verdict it never saw. That is safe and bounded by the same retry budgets, but it does cost one repeated execution. In practice arm 1 now reaches these runs first: a stranded outbox row is reclaimed and delivered with its state updates intact, so the run recovers on the verdict that was actually committed and arm 3 finds nothing left to repair.

#### The outbox processing lease

Arm 1 marks a row `processing` and commits before calling out to the graph runtime, because the delivery can reach GitHub and must not be held inside a database transaction. A drainer that dies mid-delivery therefore cannot release its own row, and a drain that selected `pending` alone dropped that transition forever ([#96](https://github.com/alexandre-leites/moirai/issues/96)). `processing` is consequently a lease, not a state: the claim stamps `processing_started_at`, and any drainer reclaims a claim older than 90 seconds. The inline delivery `accept_event` performs takes the same lease, so a maintenance tick landing between its commit and its delivery does not deliver the same transition twice; a drainer that loses the claim simply does nothing, because the holder is delivering the identical transition. Delivering twice is harmless in any case — nodes adopt the execution request they already have — but it buys nothing.
