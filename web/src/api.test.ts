...
describe("createApiClient request shapes", () => {
  ...
  it("unwraps the collection envelopes the API returns", async () => {
    const projects = [{ id: "p1", name: "svc", enabled: true, requiredRunnerLabels: ["go"], pipelineSteps: [], executionImage: "", requireHumanApproval: false, requirePlanning: false, skipChecks: false, taskSources: [] }];
