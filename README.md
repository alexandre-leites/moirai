...
- The provider circuit gates scheduling for every project on that provider, so a sync pass decides it once, at the end: it is cleared only when the pass had a success and no failure, and a refused `agent:*` label write backs that project's sync off without opening it.
