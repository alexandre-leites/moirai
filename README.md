...
- The provider circuit gates scheduling for every project on that provider, so a sync pass decides it once, from the pass as a whole: any project that syncs clears it, and only a pass in which every attempted project failed records a failure against it. One broken project among healthy ones backs itself off instead of halting the provider, and a refused `agent:*` label write never counts against it.
