...
| `LOOP_RUNNER_MAX_CONTINUATIONS` | `3` | How many times one execution may re-engage its agent after the goal gate finds the objective unmet. `0` switches the goal loop off; the maximum is `10`. Continuations run inside the same lease and share the packet's `timeoutSeconds` deadline, so this changes neither the orchestrator's budgets nor an execution's wall-clock *bound* — though a continuing execution does occupy its runner and project slot for longer within that bound. The orchestrator sends `timeoutSeconds: 0` today, which sets no bound at all and leaves no remaining budget to fund a continuation, so the loop stops after the first attempt ([#276](https://github.com/alexandre-leites/moirai/issues/276)). |
| `LOOP_RUNNER_AGENT_SILENCE_TIMEOUT` | `10m` | How long one agent attempt may run without producing any output before the runner terminates it and the goal gate re-engages the agent ([#410](https://github.com/alexandre-leites/moirai/issues/410)). A silent agent is wedged rather than working, so waiting out the whole packet timeout on one wastes the execution; `0` disables the bound. |
...
An agent that is not told what it has probes for it, and a probe that fails still costs a whole attempt — and attempts are scarce. Under the Python orchestrator's budgets, a planning role got two, so two runs that ended in `/bin/sh: python3: not found` blocked a workflow permanently ([#218](https://github.com/alexandre-leites/moirai/issues/218)). V1 is harsher rather than kinder: a single non-delivering execution parks the issue until a human retries it (`parkIssue`, `orchestrator/internal/server/server.go`). The toolchain is therefore *declared* to the agent rather than discovered by it.
...
Every agent execution must write the result document named by the task packet's `expectedOutput` (default `.loop/result.json`, validated against `schemas/agent-result.schema.json`). The runner treats it as the only evidence of what the agent did: it must be valid JSON with `protocolVersion` `1.0`, an `executionId` matching the execution, a non-empty `summary`, and a `status` of `completed`, `blocked`, or `failed`. A planner execution is the one exception: its deliverable is the plan, not a code change, so it reports `status: planned`, which the runner accepts for the planner role and reports as a `completed` terminal event so the orchestrator folds the plan into the developer packet.
...
- The terminal event type stays `failed` — the vocabulary is a shared contract with the orchestrator (`validEventType`, `orchestrator/internal/server/server.go`), the transport, and `app.jobs.status` — and the payload carries `status: "blocked"` with `blocked: true`.
...
The Go V1 orchestrator reads the other half of that contract (`persistExecutionEvent` in `orchestrator/internal/server/server.go`). The event is still stored under its own type — `failed`, with the whole payload preserved verbatim in `app.workflow_events` — but the *run* is ended at the terminal status the payload declares: a `failed` event marked `blocked: true` terminates the run as `blocked`, with `blocking_reason` and `terminal_reason` composed from the agent's `summary` and `remainingWork` and bounded to the 1 KiB that column carries elsewhere. The two share that budget rather than spending it first-come: a verbose summary cannot crowd the remaining work out of the reason, because the list is the half a human reads to decide what to do next. That is what puts the run in the console's needs-attention triage (`waiting_human ∪ blocked`, `web/src/status.ts`) rather than leaving it filed as an anonymous failure.

The flag is the contract, and only the flag: a payload without `blocked: true` — a crash, a `status: "blocked"` written by hand, a malformed or wrongly typed field — still ends the run as `failed`. Misreading a crash as a deliberate block would be worse than the bug that behaviour replaced, because it files a real failure where nobody is looking for one. Project-lock release and issue parking are identical for both: a block is a non-delivering outcome like any other, and only a manual retry makes the issue schedulable again.
...
**Bounds.** The loop terminates on five independent limits: the continuation budget, the loop guard, the execution context (a cancelled or expired lease is never answered with another agent process), a result document that cannot be set aside, and the packet's `timeoutSeconds`, which bounds the *total* agent wall clock rather than each invocation — the first run receives the whole packet timeout and every continuation only what is left, and a remainder too small to fund a real run ends the loop instead. Each attempt is additionally bounded by `LOOP_RUNNER_AGENT_SILENCE_TIMEOUT`: an agent that produces no output for that long is wedged rather than working, so the runner terminates it and the gate re-engages it instead of waiting out the whole packet timeout on a process that stopped talking ([#410](https://github.com/alexandre-leites/moirai/issues/410)). Because the total is unchanged, lease renewal needs nothing new: `StreamSupervisor`'s heartbeat drives `ControlLoop.Reconcile` → `OfferState.RenewDue` on its own ticker while executions run in their own goroutines, so renewal never depended on how long an execution takes, and an execution cannot outlive whatever bound its packet sets.

**A packet that sets no bound is not bounded here.** The orchestrator hardcodes `timeoutSeconds: 0` and the runner accepts it ([#276](https://github.com/alexandre-leites/moirai/issues/276)), so in practice there is no agent wall-clock bound at all today: `execution.Supervisor` and `execution.DockerExecutor` attach a deadline only when the timeout is positive, and the agent process runs until it exits, is cancelled, or the lease lapses. A zero timeout also puts the loop's shared deadline in the past before the first attempt finishes, so no continuation is ever started and any undelivered run reports `not delivered (execution time budget exhausted)`. The remaining backstop is the orchestrator's lease sweep, which reclaims a job whose runner stopped renewing — that catches a *dead* runner, not a live one holding a hung agent. Giving the packet a real timeout is #276's job, not the goal loop's.
...
The orchestrator remains authoritative: it fences every event on the lease generation. Note that it currently *rejects* an event whose lease has expired — the recovery sweep's `reclaimExpiredLeases` (`orchestrator/internal/server/recovery.go`) bumps the generation, so a terminal event reported after expiry is discarded and the control stream is aborted. The runner still records and offers the outcome; accepting it as recovery evidence is tracked as the orchestrator half of #93.
