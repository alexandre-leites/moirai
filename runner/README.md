...
The Go V1 orchestrator persists the event and its payload verbatim and ends the run at the terminal event's own type — a block therefore lands as `failed`, with `blocked: true`, the summary and the remaining work preserved in the stored `app.workflow_events` payload (`persistExecutionEvent` in `orchestrator/internal/server/server.go`). Routing it to the terminal `blocked` status with a `blocking_reason` composed from the agent's account is target scope, not current behavior.
