...
An agent that is not told what it has probes for it, and a probe that fails still costs a whole attempt. Attempts are scarce — the planning phase is specified to allow two — so two runs that ended in `/bin/sh: python3: not found` block a workflow permanently ([#218](https://github.com/alexandre-leites/moirai/issues/218)). The toolchain is therefore *declared* to the agent rather than discovered by it.
