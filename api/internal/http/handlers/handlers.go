...
func pipelineSteps(steps []pipelineStepRequest) []*controlv1.PipelineStep {
	result := make([]*controlv1.PipelineStep, len(steps))
	for index, step := range steps {
		result[index] = &controlv1.PipelineStep{Command: step.Command, TimeoutSeconds: step.TimeoutSeconds, Position: step.Position, Required: step.Required}
	}
	return result
}
...
func (h *ProjectHandlers) createProject(w http.ResponseWriter, r *http.Request) {
	var body struct {
		...
		PipelineSteps        []pipelineStepRequest `json:"pipelineSteps"`
	}
...
func projectPayload(p *controlv1.Project) map[string]any {
	...
	pipelineSteps := make([]map[string]any, len(p.PipelineSteps))
	for i, step := range p.PipelineSteps {
		pipelineSteps[i] = map[string]any{"command": step.Command, "timeoutSeconds": step.TimeoutSeconds, "position": step.Position, "required": step.Required}
	}
